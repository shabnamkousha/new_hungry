...
private let defaultBackgroundCardFrameAnimationDuration: TimeInterval = 0

//Opacity values
private let defaultAlphaValueOpaque: CGFloat = 1
private let defaultAlphaValueTransparent: CGFloat = 0.0
private let defaultAlphaValueSemiTransparent: CGFloat = 0
...
open class KolodaView: UIView, DraggableCardDelegate {
    ...
    private func layoutCard(_ card: DraggableCardView, at index: Int) {
        if index == 0 {
            ...
            if scale.width == 0 && scale.height == 0{
                card.layer.transform = CATransform3DScale(CATransform3DIdentity, 0.001, 0.001, 1.0)
            }else{
    ...
    internal func moveOtherCardsWithPercentage(_ percentage: CGFloat) {
        ...
        for index in 1..<visibleCards.count {
          ...
          let card = visibleCards[index]
            ...
            if scale.width == 0 && scale.height == 0 {
                card.layer.transform = CATransform3DScale(CATransform3DIdentity, 0.01, 0.01, 1.0)
            }else{
                card.layer.transform = CATransform3DScale(CATransform3DIdentity, scale.width, scale.height, 1.0)
            }
    ...
    private func loadNextCard() {
        ...
        let scale = cardParameters.scale
        if scale.width == 0 && scale.height == 0 {
            lastCard.layer.transform = CATransform3DScale(CATransform3DIdentity, 0.001, 0.001, 1)
        }else{
